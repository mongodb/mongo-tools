...
// should the logging be non debug?
func (dump *MongoDump) DumpPreludeMetadata() error {
	...
	if err != nil {
		log.Logvf(log.Always, "error in prelude: 1 %v", err)
	...
	if err != nil {
		log.Logvf(log.Always, "error in prelude: 2 %v", err)
		return fmt.Errorf("error marshaling json")
	}

	_, err = writer.Write(bytes)
	if err != nil {
		log.Logvf(log.Always, "error in prelude: 3 %v", err)
