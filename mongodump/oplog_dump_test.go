...
// TestOplogDumpRejectsRename checks that a --oplog dump fails when a collection
// is renamed while it runs. A rename is not replayable against a dump taken
// around it, so the resulting oplog would not restore.
func TestOplogDumpRejectsRename(t *testing.T) {
	...
	_, err = testDB.Collection(fromColl).InsertOne(context.Background(), bson.D{{"x", 1}})
...
// TestOplogDumpRejectsAdminSystemVersionChange checks that a --oplog dump fails
// when admin.system.version is written while it runs. That collection holds the
// FCV document, so a change to it mid-dump means the dump spans two different
// server feature sets and the oplog cannot be replayed as one unit.
func TestOplogDumpRejectsAdminSystemVersionChange(t *testing.T) {
	...
	// The real FCV document lives here, so this writes and removes a document of
	// its own rather than touching anything the server put there.
	systemVersion := client.Database("admin").Collection("system.version")
	dummyID := "mongodump-oplog-test"
	t.Cleanup(func() {
...
// TestOplogDumpFailsOnStandalone checks that --oplog against a standalone fails
// before writing anything. A standalone keeps no oplog, so there is nothing for
// mongodump to capture and no point-in-time dump it can produce.
func TestOplogDumpFailsOnStandalone(t *testing.T) {
	...
	require.ErrorContains(
		...
		"a --oplog dump of a server with no oplog fails",
	)
...
//
// The JS tests these replace raced the modification against an unpaused dump and
// asserted only on a nonzero exit code, so they could pass without the
// modification ever reaching the window.
