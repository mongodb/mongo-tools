...
// invalidDumpOptionsCases collects the option combinations mongodump rejects.
// Each case asserts on the error validation returns, rather than on a process
// exit code, so it pins down which combination was rejected and why.
