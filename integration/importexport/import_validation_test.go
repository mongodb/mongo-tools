...
func (s *ImportExportSuite) TestImportDocumentValidation() {
	...
	_, err := db.Collection(collName).InsertMany(s.T().Context(), docs)
	...
	s.Run("no validator imports all docs", func() {
		...
		n, err := db.Collection(collName).CountDocuments(s.T().Context(), bson.D{})
		s.Require().NoError(err)
		s.EqualValues(1000, n, "import without validation should import all 1000 documents")
	})
	...
	s.Run("validator skips invalid docs", func() {
		...
		n, err := db.Collection(collName).CountDocuments(s.T().Context(), bson.D{})
		s.Require().NoError(err)
		s.EqualValues(500, n, "only valid documents should be imported")
	})
