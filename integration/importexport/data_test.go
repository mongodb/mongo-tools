...
func (s *ImportExportSuite) TestRoundTripBasicData() {
	...
	s.Require().NoError(err)
	s.EqualValues(50, imported, "should import all 50 documents")

	count, err := coll.CountDocuments(s.T().Context(), bson.D{})
	s.Require().NoError(err)
	s.EqualValues(50, count, "collection should have all 50 documents after round-trip")

	for i := range 50 {
		c, err := coll.CountDocuments(s.T().Context(), bson.D{{"_id", i}})
		s.Require().NoError(err)
		s.EqualValues(1, c, "document with _id %d should exist after round-trip", i)
	}
}
...
func (s *ImportExportSuite) TestRoundTripDataTypes() {
	...
	imported, _, err := mi.ImportDocuments()
	s.Require().NoError(err)
	s.EqualValues(9, imported, "should import all 9 documents")

	count, err := coll.CountDocuments(s.T().Context(), bson.D{})
	s.Require().NoError(err)
	s.EqualValues(9, count, "collection should have all 9 documents after round-trip")
	...
	for _, q := range []bson.D{
		...
		c, err := coll.CountDocuments(s.T().Context(), q)
		s.Require().NoError(err)
		s.EqualValues(1, c, "document matching %v should exist after round-trip", q)
	}
}
...
func (s *ImportExportSuite) TestRoundTripDecimal128() {
	...
	coll := client.Database(dbName).Collection(collName)
	_, err = coll.InsertOne(s.T().Context(), testDoc)
	s.Require().NoError(err)
	...
	imported, _, err := mi.ImportDocuments()
	s.Require().NoError(err)
	s.EqualValues(1, imported, "should import 1 document")

	var result bson.D
	err = coll.FindOne(s.T().Context(), bson.D{{"_id", "foo"}}).Decode(&result)
	s.Require().NoError(err)
	s.Equal(testDoc, result, "imported doc should match original")
}
...
func (s *ImportExportSuite) TestRoundTripViewExport() {
	...
	_, err := db.Collection("cities").InsertMany(s.T().Context(), cities)
	s.Require().NoError(err)
	...
	for _, view := range []struct{ name, state string }{
		...
		err = db.CreateView(s.T().Context(), view.name, "cities", pipeline)
		s.Require().NoError(err)
	}

	n, err := db.Collection("citiesID").CountDocuments(s.T().Context(), bson.D{})
	s.Require().NoError(err)
	s.EqualValues(3, n, "should have 3 cities in Idaho view")
	n, err = db.Collection("citiesNY").CountDocuments(s.T().Context(), bson.D{})
	s.Require().NoError(err)
	s.EqualValues(2, n, "should have 2 cities in New York view")
	n, err = db.Collection("citiesCA").CountDocuments(s.T().Context(), bson.D{})
	s.Require().NoError(err)
	s.EqualValues(4, n, "should have 4 cities in California view")
	...
	s.Require().NoError(db.Drop(s.T().Context()))
	...
	imported, _, err := mi.ImportDocuments()
	s.Require().NoError(err)
	s.EqualValues(4, imported, "import should succeed")

	n, err = db.Collection("CACities").CountDocuments(s.T().Context(), bson.D{})
	s.Require().NoError(err)
	s.EqualValues(4, n, "restored view should have correct number of rows")
}
