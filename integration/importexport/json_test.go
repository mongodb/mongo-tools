...
func (s *ImportExportSuite) TestRoundTripFieldsJSON() {
	...
	n, err = dest.CountDocuments(s.T().Context(), bson.D{{"b", 1}})
	s.Require().NoError(err)
	s.EqualValues(0, n, "b=1 should not have been exported")
	n, err = dest.CountDocuments(s.T().Context(), bson.D{{"b", 2}})
	s.Require().NoError(err)
	s.EqualValues(0, n, "b=2 should not have been exported")
	n, err = dest.CountDocuments(s.T().Context(), bson.D{{"c", 3}})
	s.Require().NoError(err)
	s.EqualValues(0, n, "c=3 should not have been exported")
	...
	n, err = dest.CountDocuments(s.T().Context(), bson.D{{"b", 1}})
	s.Require().NoError(err)
	s.EqualValues(1, n, "1 document should have b=1")
	n, err = dest.CountDocuments(s.T().Context(), bson.D{{"b", 2}})
	s.Require().NoError(err)
	s.EqualValues(1, n, "1 document should have b=2")
	n, err = dest.CountDocuments(s.T().Context(), bson.D{{"c", 3}})
	s.Require().NoError(err)
	s.EqualValues(1, n, "1 document should have c=3")

	var fromSource, fromDest bson.M
	q := bson.D{{"a", 1}, {"b", 1}}
	err = db.Collection("source").FindOne(s.T().Context(), q).Decode(&fromSource)
	s.Require().NoError(err)
	err = dest.FindOne(s.T().Context(), q).Decode(&fromDest)
	s.Require().NoError(err)
	s.Equal(
...
func (s *ImportExportSuite) TestRoundTripJSONArray() {
	...
	s.Error(err, "import without --jsonArray should fail on jsonArray output")

	n, err := coll.CountDocuments(s.T().Context(), bson.D{})
	s.Require().NoError(err)
	s.EqualValues(0, n, "nothing should have been imported without --jsonArray")
	...
	s.Require().NoError(err)
	s.EqualValues(20, imported, "should import all 20 documents with --jsonArray")

	n, err = coll.CountDocuments(s.T().Context(), bson.D{})
	s.Require().NoError(err)
	s.EqualValues(20, n, "all 20 documents should be present after import")
	for i := range 20 {
		c, err := coll.CountDocuments(s.T().Context(), bson.D{{"_id", i}})
		s.Require().NoError(err)
		s.EqualValues(1, c, "document with _id %d should exist", i)
	}
}
