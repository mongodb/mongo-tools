...
// according to whether the replica set can satisfy the write concern it was given.
...
// timeout makes mongoimport wait for the secondaries rather than giving up. It
// tests waiting rather than failing, so it is one of the two cases that have to run
// the tool concurrently with the failpoint.
...
// TestMongoFilesWaitsForSlowReplication is the same for mongofiles. It writes
// through GridFS rather than an insert, so the waiting is not the same code as the
// import's.
...
// database is dropped before replication is stopped, because a drop needs the whole
// set and so cannot happen while the secondaries are held back.
...
// writeConcernCase is one row of the matrix: some number of secondaries stopped
// from replicating, a write concern, and whether the tool is expected to fail.
...
// writeConcernCases is the matrix these tests run. The write concerns that
...
// do not matter beyond being a valid import.
func (s *WriteConcernSuite) writeImportFile() string {
	...
	s.Require().NoError(os.WriteFile(path, lines, 0o600), "can write the import file")
