...
// TestRoundTripBSONTypeEdges checks that the BSON types with the most awkward extended-JSON
// representations survive an export-then-import round trip byte for byte. Comparing whole documents
// rather than field by field catches a type that comes back as the wrong width or loses its
// wrapper.
...
// TestRoundTripMinKeyMaxKeyIDs checks that MinKey and MaxKey survive the round trip in the one
// position where a wrong type breaks the document rather than just a field: the _id.
...
// TestRoundTripUndefinedInArrays checks that an array holding undefined elements round trips with
// the array's shape intact, in both the line-per-document and --jsonArray output formats. The two
// documents cover an undefined-bearing array in a field of its own and one next to a scalar field.
	...
	for _, jsonArray := range []bool{false, true} {
		s.Run("jsonArray="+strconv.FormatBool(jsonArray), func() {
			...
			s.Assert().EqualValues(
				...
				"both documents are imported",
			)

			s.Assert().Equal(
				sourceDocs,
				s.docsSortedByID(db.Collection("dest")),
				"each array's undefined elements keep their type and position",
			)
		})
	}
}

// TestExportQueryOnNonFiniteDoubles checks that a --query naming NaN or an infinity in canonical
// extended JSON reaches the server as the double it denotes, so it selects exactly the documents
// holding that value.
