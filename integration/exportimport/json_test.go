...
// NumberInt(5) or a bare hex literal.
...
// TestImportLegacyExtendedJSONTypes checks that `--legacy` accepts every legacy extended-JSON
// spelling of a BSON value and stores each one as the value and the type that spelling
// denotes. Comparing the decoded values rather than querying by $type catches a hex literal parsed
// as 0 or a Date(45) parsed as the epoch, both of which still have the right type. Because the Go
// type of each expected value is the BSON type it decodes from, the comparison also covers the type
...
// TestRoundTripJSONArrayOverMaxBSONSize checks that `--jsonArray` handles an export whose single
// array is far larger than the 16MB maximum BSON document size, in both directions: mongoexport has
// to stream the array out rather than build it, and mongoimport has to read documents out of it
// incrementally.
