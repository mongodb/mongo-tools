...
// docsSortedByID returns every document in coll as a bson.D, ordered by _id so that a comparison
// against the documents that went in is a total order.
func (s *ExportImportSuite) docsSortedByID(coll *mongo.Collection) []any {
	...
	var docs []any
	s.Require().NoError(cursor.All(s.Context(), &docs))
