...
// embedded double quotes, values that are nothing but punctuation, and a field name containing a
// space.
var csvPunctuationDoc = bson.D{
	{"a", int32(1)},
	{"b", `foo,bar"baz,qux`},
	{"c", int32(5)},
	{"d d", int32(-6)},
...
// This is a string containing a comma-separated list of the field names.
var csvPunctuationDocFieldNames = strings.Join(
	slices.Sorted(slices.Values(lo.Map(csvPunctuationDoc, func(e bson.E, _ int) string {
		return e.Key
	}))),
	",",
)
...
var csvTextEdgeCaseDocs = []bson.D{
	{
		{"a", int32(1)},
		{
			"b",
	...
	},
	{
		{"a", int32(4)},
		{"b", ""},
		{"c", "How are empty entries handled?"}},
	{
		{"a", int32(5)},
		{"b", `""`},
...
// TestCSVImportTextEdgeCases checks how the CSV parser handles newlines and commas inside quoted
// cells, doubled quotes, backslashes, empty cells, and padding around quoted cells.
func (s *ExportImportSuite) TestCSVImportTextEdgeCases() {
	...
	s.Run("fields option treats the header row as data", func() {
		...
		s.Require().Len(
			docs,
			len(csvTextEdgeCaseDocs)+1,
			"every row including the header is imported",
		)
		s.Assert().Equal(
			append(slices.Clone(csvTextEdgeCaseDocs), bson.D{{"a", "a"}, {"b", "b"}, {"c", "c"}}),
			docs,
...
// tsvTextEdgeCases has an empty first cell, which is what distinguishes an empty TSV cell from a
// missing one.
const tsvTextEdgeCases = "a\tb\tc\td\te\n" +
	"\t1\tfoobar\t5\t-6\n"

var tsvTextEdgeCasesDoc = bson.D{
	{"a", ""},
	{"b", int32(1)},
	{"c", "foobar"},
	{"d", int32(5)},
...
// TestTSVImport checks that a tab-delimited file imports with both an explicit --fields list and
// --headerline.
...
// TestCSVExportFormatsBSONTypes checks how mongoexport renders BSON types that have no CSV
// equivalent, and that mongoimport can read those renderings back.  Exporting to CSV is lossy by
// design, so the export half pins the exact text of each cell instead of expecting the values to
// survive.
...
// importDelimited imports path into ns as the given type, either csv or tsv. fields names the
// columns; passing an empty fields uses the file's first line as the header instead.
...
// docsSortedByFieldA returns the documents in coll ordered by their a field, with their _ids
// removed so that they can be compared against expected documents. Every input here keys its rows
// on a and no two rows share a value, so the order is total. Where a row's a is a number, BSON's
// ordering of numbers ahead of strings puts an imported header row last; the TSV input instead
// relies on its empty first cell sorting before the header's "a".
func (s *ExportImportSuite) docsSortedByFieldA(coll *mongo.Collection) []bson.D {
	...
	for i := range docs {
		bsonutil.RemoveKey("_id", &docs[i])
	}

	return docs
}

// csvText renders each record as CSV the way mongoexport does, so a test can spell out the cells it
// expects without hand-escaping the quotes and commas in them.
func (s *ExportImportSuite) csvText(records ...[]string) string {
	var buf strings.Builder
	writer := csv.NewWriter(&buf)
	for _, record := range records {
		s.Require().NoError(writer.Write(record), "can write an expected CSV record")
	}
	writer.Flush()
	s.Require().NoError(writer.Error(), "can flush the expected CSV")
