...
// csvTextEdgeCases is the fixture the legacy csvimport1.js read from
// jstests/tool/data/csvimport1.csv, byte for byte. The irregular whitespace is
// the point: cells are variously padded outside their quotes, and the parser has
// to strip that padding but keep whatever is inside the quotes.
...
func (s *ExportImportSuite) TestCSVImportTextEdgeCases() {
	...
	testDB := s.Client().Database(dbName)
	csvPath := s.writeFixture("csvimport1.csv", csvTextEdgeCases)
...
// tsvFixture is the fixture the legacy tsv1.js read from
// jstests/tool/data/a.tsv. Its first cell is empty, which is what distinguishes
// an empty TSV cell from a missing one.
const tsvFixture = "a\tb\tc\td\te\n" +
	"\t1\tfoobar\t5\t-6\n"

var tsvFixtureDoc = bson.M{
	"a": "",
	"b": int32(1),
	"c": "foobar",
	"d": int32(5),
...
func (s *ExportImportSuite) TestTSVImport() {
	...
	testDB := s.Client().Database(dbName)
	tsvPath := s.writeFixture("a.tsv", tsvFixture)
	...
	s.Run("fields option treats the header row as data", func() {
		...
		docs := s.docsSortedByA(testDB.Collection("with_fields"))
		s.Require().Len(docs, 2, "the data row and the header row are both imported")
		s.Assert().Equal(tsvFixtureDoc, docs[0], "the data row imports unchanged")
		s.Assert().Equal(
...
func (s *ExportImportSuite) TestCSVExportFormatsBSONTypes() {
	...
	s.Require().NoError(err, "can decode the fixture's binary payload")
	timestamp, err := time.Parse(time.RFC3339Nano, "2009-08-27T12:34:56.789Z")
	s.Require().NoError(err, "can parse the fixture's date")
	...
	// Importing the export back is the other half of what the legacy test checked:
	// mongoexport writes cells that mongoimport has to be able to parse, awkward
	// ones included. None of the original types survive, because the CSV holds only
...
// Every fixture here keys its rows on a and no two rows share a value, so the
// order is total. Where a row's a is a number, BSON's ordering of numbers ahead
// of strings puts an imported header row last; the TSV fixture instead relies on
...
// writeFixture writes content to a temp file named name and returns its path.
func (s *ExportImportSuite) writeFixture(name, content string) string {
