...
func (s *ImportExportSuite) TestRoundTripFieldFile() {
	...
	s.Require().NoError(err)
	s.EqualValues(0, n, "c=3 should not have been exported (not in fieldFile)")
}
...
func (s *ImportExportSuite) TestRoundTripFieldsCSV() {
	...
	s.Require().NoError(err)
	s.NotEqual(fromSource["_id"], fromDest["_id"], "_id should not have been exported")
}
...
func (s *ImportExportSuite) TestRoundTripNestedFieldsCSV() {
	...
	for _, tc := range []struct {
		...
		n, err := dest.CountDocuments(s.T().Context(), tc.filter)
		s.Require().NoError(err)
		s.EqualValues(tc.count, n, tc.msg)
	}
}
