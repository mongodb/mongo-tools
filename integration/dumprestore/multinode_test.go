...
// count.
//
// The number comes from the set's own membership rather than being written into
// the test: on a three-node set, a fixed w:2 is satisfied by the primary and
// whichever secondary happens to reply first, so the secondary this test reads
// from could lag and fail it.
func (s *DumpRestoreSuite) TestRestoreWithWriteConcernForWholeSet() {
	...
	// below pass on the original documents rather than on anything the restore
	// wrote.
	s.Require().Zero(
...
func (s *DumpRestoreSuite) dumpWithArgs(hostArgs []string, dumpArgs ...string) {
	opts, err := mongodump.ParseOptions(slices.Concat(hostArgs, dumpArgs), "", "")
...
func (s *DumpRestoreSuite) restoreWithArgs(hostArgs []string, restoreArgs ...string) {
	restore, err := getRestoreWithHostArgs(hostArgs, restoreArgs...)
