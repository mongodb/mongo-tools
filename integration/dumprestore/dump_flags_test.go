...
func (s *DumpRestoreSuite) testDumpExcludeCollectionsWithPrefix() {
	...
	// One victim has a dot in its name, so that the prefix is shown to match on
	// a name where it is not the whole first dot-separated part.
...
func (s *DumpRestoreSuite) TestDumpToStdout() {
	...
	// Nothing is written through this handle -- mongodump inherits it and writes
	// to it as its own standard output -- so closing it is only cleanup.
