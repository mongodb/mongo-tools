...
// viewCities holds one city per document, with a view per state. The
...
func (s *DumpRestoreSuite) createCitiesAndViews(testDB *mongo.Database) {
	docs := lo.Map(viewCities, func(c city, _ int) any {
		return bson.D{{"_id", c.city}, {"state", c.state}}
	})
...
// assertRestoredEveryCity checks the restore inserted one document per city.
// That count holds in both dump modes: a default dump carries the cities once in
// the backing collection and the views carry no data of their own, while
...
func (s *DumpRestoreSuite) assertCitiesRestored(testDB *mongo.Database) {
	s.Assert().ElementsMatch(
		lo.Map(viewCities, func(c city, _ int) string {
			return c.city
		}),
...
func (s *DumpRestoreSuite) assertViewsMatchTheirState(testDB *mongo.Database) {
	for _, state := range viewStates {
		wantCities := lo.FilterMap(viewCities, func(c city, _ int) (string, bool) {
			return c.city, c.state == state
		})
