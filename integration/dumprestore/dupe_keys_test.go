...
const duplicateKeysDocCount = 50

var duplicateKeysRemovedIDs = mapset.NewSet(0, 5, 6, 9, 12, 27, 40, 46, 47, 49)
...
func (s *DumpRestoreSuite) assertDuplicateKeyRestoreFillsGaps(
	...
	coll := s.createCollectionWithSequentialIDs(testDB)
	...
	s.withBSONMongodump(func(dir string) {
		...
		s.Assert().EqualValues(
			duplicateKeysRemovedIDs.Cardinality(),
			result.Successes,
			"only the missing documents are inserted",
		)
		s.Assert().EqualValues(
			duplicateKeysDocCount-duplicateKeysRemovedIDs.Cardinality(),
			result.Failures,
...
func (s *DumpRestoreSuite) createCollectionWithSequentialIDs(
...
func (s *DumpRestoreSuite) removeAndMarkDocuments(coll *mongo.Collection) {
	deleted, err := coll.DeleteMany(
		s.Context(),
		bson.D{{"_id", bson.D{{"$in", duplicateKeysRemovedIDs.ToSlice()}}}},
	)
	s.Require().NoError(err, "can delete the documents under test")
	s.Require().EqualValues(
		duplicateKeysRemovedIDs.Cardinality(),
	...
	updated, err := coll.UpdateMany(
		s.Context(),
		bson.D{},
		bson.D{{"$set", bson.D{{"survived", true}}}},
	)
...
func (s *DumpRestoreSuite) assertSurvivorsUntouched(coll *mongo.Collection) {
	...
	for _, doc := range restored {
		if duplicateKeysRemovedIDs.Contains(doc.ID) {
			s.Assert().False(
				doc.Survived,
