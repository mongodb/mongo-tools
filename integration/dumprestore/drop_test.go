...
// replacementDocCount differs from namespaceDocCount so that a restore which failed to drop the
// replacements is visible in the document count as well as in the document contents.
const replacementDocCount = 20

// TestRestoreDropWithData checks that --drop replaces the existing contents of every restored
// collection. The pre-dump data is swapped for a disjoint set of documents of a different size
// before the restore, so a restore that upserted by _id instead of dropping would leave both sets
// behind.
...
// TestRestoreDropOneCollection checks that --drop scoped with --collection replaces only that
// collection, leaving its sibling and a second database holding the same collection names
// untouched.
func (s *DumpRestoreSuite) TestRestoreDropOneCollection() {
	const (
		restoredCollName = "coll1"
		siblingCollName  = "coll2"
	)

	collNames := []string{restoredCollName, siblingCollName}
	...
	s.assertOnlyReplacements(sourceDB.Collection(siblingCollName))
	for _, collName := range collNames {
		s.assertOnlyReplacements(otherDB.Collection(collName))
	}
}

// TestRestoreDropNonexistentDB checks that --drop against a database that does not exist is not an
// error and restores normally.
...
// replaceCollectionContents swaps a collection's documents for a disjoint set, so that a restore
// which did not drop first leaves both sets behind and a restore which dropped leaves only the
// dumped ones.
