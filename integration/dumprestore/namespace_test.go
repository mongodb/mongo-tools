...
func (s *DumpRestoreSuite) createNamespaceSourceDBs(sourcePrefix string) {
	...
	for _, sourceDB := range namespaceSourceDBs {
		...
		for _, collNum := range sourceDB.collNums {
			coll := testDB.Collection(namespaceCollName(collNum))
	...
	for _, sourceDB := range namespaceSourceDBs {
		for _, collNum := range sourceDB.collNums {
			bsonFile := filepath.Join(
				...
				namespaceCollName(collNum)+".bson",
			)
...
func (s *DumpRestoreSuite) restoreAndAssertNamespaceMapping(mappingCase namespaceMappingCase) {
	...
	for _, sourceDB := range namespaceSourceDBs {
		for _, collNum := range sourceDB.collNums {
			collName := namespaceCollName(collNum)
...
// namespaceCollName is the collection a source database's collection number names. The filters
// under test select collections by this number, so the number is what the case table stores.
func namespaceCollName(collNum string) string {
	return "coll" + collNum
}
