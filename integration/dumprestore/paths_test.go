...
// specialCollNames hold characters that have to be escaped to become filenames in a
// directory-format dump: a slash would otherwise create a subdirectory, and a percent is the escape
// character itself.
var specialCollNames = []string{"coll/foo", "coll%bar", "coll%2Fbaz"}

// TestRestoreSpecialCollectionNames round-trips collections whose names cannot be used verbatim as
// filenames. The archive format carries the name as data and is covered by `TestPipedDumpRestore`;
// this covers the directory format, where the name has to survive being escaped into a filename and
// unescaped again. The `coll%2Fbaz` name is a double-escaping guard: if `%` ever stopped being
// escaped on the dump side, it would unescape back to `coll/baz` and restore into the wrong
// namespace.
...
// TestArchiveThroughStdout runs a real mongodump process with `--archive` and pipes its standard
// output into a real `mongorestore` process reading from standard input. `TestPipedDumpRestore`
// covers the archive format itself, but it connects the two in-process through an `io.Pipe`. This
// test exercises the real CLI plumbing, passing `--archive` with no value to `mongodump` so that
// the archive contents are written to stdout. It also passes `--archive` with no value to
// `mongorestore` so that it reads from stdin.
//
// The restore writes back over the same namespaces with --drop, which is what makes the escaped
// collection names worth including here: they have to survive the archive as well as a drop and
// recreate.
func (s *DumpRestoreSuite) TestArchiveThroughStdout() {
	...
	s.T().Cleanup(func() {
		// A failed assertion below aborts the test where it fails, so mongorestore would
		// otherwise be left unreaped. On the path where it was already waited for, killing it
		// reports that it is done, which is not worth warning about.
		if err := restore.Process.Kill(); err != nil && !errors.Is(err, os.ErrProcessDone) {
			s.T().Logf("could not kill mongorestore: %v", err)
		}
		_ = restore.Wait()
	})
	...
	// The restore writes back over the namespaces it read from, so the documents being present
	// afterwards does not by itself show that anything came through the pipe. The count
	// mongorestore reports does.
	s.Require().Regexp(
		fmt.Sprintf(
...
// TestRestoreDumpWithSymlinks restores a dump directory in which a database directory and a
// collection's bson file are both symlinks, and which also contains a symlink to a plain file at
// the top level. The last one is the point of the test: a symlinked regular file must not be walked
// as though it were a database directory.
func (s *DumpRestoreSuite) TestRestoreDumpWithSymlinks() {
	...
	// The real contents live outside the dump directory, so the only way mongorestore can reach
	// them is by following the links.
	...
	// The insert count above is what proves the symlinked plain file was skipped: had mongorestore
	// walked it as a database directory it would either have errored or restored a third
	// collection's worth of documents.
...
// writeNamespacedBSONFile writes the same documents insertNamespacedDocs would have inserted, so
// the restored data can be checked with assertDocsCameFrom.
