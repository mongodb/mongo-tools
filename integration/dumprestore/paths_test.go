...
//
// TestRestoreSpecialCollectionNames checks that collection names holding
// characters a filename cannot carry round-trip through a directory-format dump,
// where the name has to survive being escaped into a filename and unescaped
// again. coll%2Fbaz is a double-escaping guard: if % ever stopped being escaped
// on the dump side, it would unescape back to coll/baz and restore into the
// wrong namespace.
...
func (s *DumpRestoreSuite) TestArchiveThroughStdout() {
	...
	dump := exec.Command(
		...
		append(
			...
			append(testutil.GetBareArgs(), "--archive", "--db", testDB.Name())...,
		)...,
	)
	restore := exec.Command(
		...
		append(
			...
			append(testutil.GetBareArgs(), "--archive", "--drop")...,
		)...,
	)
...
func (s *DumpRestoreSuite) TestRestoreDumpWithSymlinks() {
	...
	// This check only documents the intent.
