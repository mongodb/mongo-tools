...
func (s *DumpRestoreSuite) assertExtendedJSONQueryFilters(queryCase extendedJSONQueryCase) {
	...
	s.Require().NoError(err, "can insert the two fixture documents")
	...
	// EqualValues, because an int in the fixture comes back from the server as
	// an int32.
...
// started and no more than what existed when it finished.
func (s *DumpRestoreSuite) TestDumpForceTableScan() {
	...
	s.Assert().LessOrEqual(
		restored,
		countAfter,
		"the dump holds no more than the documents that existed when it finished",
	)
}
