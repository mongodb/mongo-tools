...
func (s *DumpRestoreSuite) testKeepIndexVersion() {
	...
	// assertion, because a v1 secondary index is silently created at the server's
	// current version. Comparing the whole spec checks the version the server
	// actually chose, whatever that is.
...
// command's idIndex argument: a v1 secondary index is silently created at the
// server's current version instead. The secondary index is still created, so the
// tests can check that its spec survives a round trip.
