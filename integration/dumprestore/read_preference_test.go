...
//
// read_preference_and_tags.js tagged every member and then asserted only that the
// tagged secondary profiled more operations than the primary, which tolerates any
// number of reads landing on the primary. Here one secondary is tagged, so it is
// the only member the read preference can select, and every other node is asserted
// to have served no read of the dumped collections at all.
//
...
var dumpTargetTag = bson.D{{"use", "dumpTarget"}}
...
func (s *DumpRestoreSuite) memberConfig(config bson.D, host string) bson.D {
	...
	s.Require().Fail("%s is one of the replica set's members", host)
