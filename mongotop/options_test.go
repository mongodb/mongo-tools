...
func TestPositionalArgumentParsing(t *testing.T) {
	...
	for _, tc := range positionalArgumentTestCases {
		...
		if tc.ExpectErr != "" {
			...
			assert.Equal(t, tc.ExpectedOpts.SleepTime, opts.SleepTime)
		...
		case "kerberos":
			assert.Equal(t, opts.Username, tc.ExpectedOpts.Username)
			assert.Equal(t, opts.Mechanism, tc.ExpectedOpts.Mechanism)
			assert.Equal(t, opts.Source, tc.ExpectedOpts.Source)
			assert.Equal(
				t,
