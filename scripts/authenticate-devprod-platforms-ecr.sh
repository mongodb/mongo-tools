...
#     function (used by the "sign" task and the SBOM generation/upload tasks) - AWS_ACCESS_KEY_ID
#     etc. are already set from that assumed role, so this logs in with them directly.
#   - Indirectly, from scripts/generate-sbom.sh. When that runs as part of an SBOM
#     generation/upload task, the task already authenticated via the path above, so this is a no-op
#     (EVG_WORKDIR is set, but AWS_ACCESS_KEY_ID isn't, since it wasn't requested for this specific
#     subprocess).
#
