...
// TestOpInsertLiveDB tests the functionality of mongotape replaying inserts
// against a live database Generates 20 recorded inserts and passes them to the
// main execution of mongotape and queries the database to verify they were
...
// TestUpdateOpLiveDB tests the functionality of mongotape replaying an update
// against a live database Generates 20 recorded inserts and an update and
// passes them to the main execution of mongotape and queries the database to
...
func TestQueryOpLiveDB(t *testing.T) {
	...
	// created to ensure that they match what we expected mongotape to have
	// executed
	for i := 0; i < numQueries; i++ {
...
// through mongotape. It generates inserts, a query, and a series of getmores
...
func TestOpGetMoreLiveDB(t *testing.T) {
	...
	go func() {
		...
		// information can be correlated by mongotape
...
func TestOpGetMoreMultiCursorLiveDB(t *testing.T) {
	...
	go func() {
		...
		// be played in mongotape's main Play function
...
func TestCommandOpFindLiveDB(t *testing.T) {
	...
	// created, and ensure that they match what we expected mongotape to have
	// executed
	for i := 0; i < numFinds; i++ {
		stat := statRec.Buffer[numInserts+i]
		t.Logf("Stat result: %#v\n", stat)
