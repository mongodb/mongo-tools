...
func TestConfigureClientAKS(t *testing.T) {
	...
	Convey("Configuring options with azure ENVIRONMENT and proper OS environments set should use the AKSCallback", t, func() {
		enabled := options.EnabledOptions{
			Auth:       true,
			Connection: true,
			Namespace:  true,
		...
		toolOptions := options.New("test", "", "", "", true, enabled)
		_, err := toolOptions.ParseArgs(
			[]string{"--uri", "mongodb://test.net/?directConnection=true&tls=true&authMechanism=MONGODB-OIDC&authMechanismProperties=ENVIRONMENT:azure"},
		)
		So(err, ShouldBeNil)
...
func TestMissConfigureClientAKS(t *testing.T) {
	...
	Convey("Configuring options with azure ENVIRONMENT and strict subset of OS environments set should be an error", t, func() {
		enabled := options.EnabledOptions{
			Auth:       true,
			Connection: true,
			Namespace:  true,
