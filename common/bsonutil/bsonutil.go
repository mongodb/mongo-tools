...
func ConvertLegacyExtJSONDocumentToBSON(doc map[string]any) error {
	for key, jsonValue := range doc {
		var bsonValue any
		var err error

		switch v := jsonValue.(type) {
		case map[string]any, bson.D: // subdocument
...
func FindValueByKey(keyName string, document *bson.D) (any, error) {
...
// key was found.
func RemoveKey(key string, document *bson.D) (any, bool) {
	if document == nil {
		return nil, false
	}
	doc := *document
...
func ParseSpecialKeys(special any) (any, error) {
...
func parseNumberLongField(jsonValue any) (int64, error) {
	switch v := jsonValue.(type) {
	case string:
		// all of decimal, hex, and octal are supported here
...
func Bson2Float64(data any) (float64, bool) {
	switch v := data.(type) {
	case int32:
