...
func (mux *Multiplexer) Run() {
	...
	for {
		...
		if index == 0 { //Control index
			...
			if EOF {
				mux.ins[index].writeChanCloseFinished <- struct{}{}
...
type MuxIn struct {
	...
	writeChanCloseFinished chan struct{}
	buf                    []byte
	hash                   hash.Hash64
...
func (muxIn *MuxIn) Open() error {
	...
	muxIn.writeChanCloseFinished = make(chan struct{})
