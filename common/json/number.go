...
func (d *decodeState) getNumberInt() interface{} {
	...
	case string:
		number = Number(v)
	default:
		d.error(fmt.Errorf("expected int32 for first argument of NumberInt constructor, got %v", v))
	}

	d.useNumber = useNumber
	arg0, err := number.Int32()
	if err != nil {
		d.error(fmt.Errorf("expected int32 for first argument of NumberInt constructor, got %v", number))
	}
...
func (d *decodeState) getNumberLong() interface{} {
	...
	default:
		d.error(fmt.Errorf("expected int64 for first argument of NumberLong constructor, got %v", v))
	...
	if err != nil {
		d.error(fmt.Errorf("expected int64 for first argument of NumberLong constructor, got %v", number))
	}
