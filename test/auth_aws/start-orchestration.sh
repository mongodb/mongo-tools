...
mongo-orchestration $ORCHESTRATION_ARGUMENTS start > orchestration/out.log 2>&1 < /dev/null &

ls -la orchestration

sleep 5
if ! curl http://localhost:8889/ --silent --show-error --max-time 120 --fail; then
  echo Failed to start mongo-orchestration, see orchestration/out.log:
  cat orchestration/out.log
  echo Failed to start mongo-orchestration, see orchestration/server.log:
