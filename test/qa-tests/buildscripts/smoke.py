...
def check_db_hashes(master, slave):
    ...
    for db in list(slave.dict.keys()):
...
at the end of testing:""")
        for coll in list(screwy_in_slave.keys()):
...
def report():
    ...
    num_missed = len(tests) - (len(winners) + len(list(losers.keys())))
    if num_missed:
...
def main():
    ...
    try:
        ...
        test_report["failures"] = len(list(losers.keys()))
