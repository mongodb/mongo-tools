(function() {
  ...
  for(var i=0;i<formats.length;i++){
    ...
    var ret = toolTest.runTool.apply(toolTest, ["import", "--file",
      "jstests/import/testdata/typed_header." + format,
      "--type=" + format,
      "--db", format + "testdb",
      "--collection", format + "testcoll",
      "--columnsHaveTypes",
      "--headerline"].concat(commonToolArgs))
    ...
    var ret = toolTest.runTool.apply(toolTest, ["import", "--file",
      "jstests/import/testdata/typed_noheader." + format,
      "--type=" + format,
      "--db", format + "testdb",
      "--collection", format + "testcoll",
      "--columnsHaveTypes",
      "--fields", header].concat(commonToolArgs))
    checkCollectionContents(c)
    reset(c)
    ...
    var ret = toolTest.runTool.apply(toolTest, ["import", "--file",
      ...
      "--collection", format + "testcoll",
      "--columnsHaveTypes",
      "--fieldFile", "jstests/import/testdata/typedfieldfile"].concat(commonToolArgs))
    checkCollectionContents(c)
    reset(c)
    ...
    var ret = toolTest.runTool.apply(toolTest, ["import", "--file",
      ...
      "--db", format + "testdb",
      "--collection", format + "testcoll",
      "--columnsHaveTypes"].concat(commonToolArgs))
    assert.neq(ret, 0)
    reset(c)

    // check that extra fields are created as expected
    var ret = toolTest.runTool.apply(toolTest, ["import", "--file",
      "jstests/import/testdata/typed_extrafields." + format,
      "--type=" + format,
      "--db", format + "testdb",
      "--collection", format + "testcoll",
      "--columnsHaveTypes",
