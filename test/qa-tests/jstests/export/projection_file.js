(function() {
  ...
  // Tests running mongoexport exporting to csv using the --projectionFile option
  jsTest.log('Testing exporting using the --projectionFile option');

  var toolTest = getToolTest('projection_file');
  ...
  // the export target
  var exportTarget = 'jstests/export/testdata/projection_file_export.json';
  ...
  // export the data, using a projecion file that specifies 'a' and 'b'
  var ret = toolTest.runTool.apply(toolTest, ['export',
    '--out', exportTarget,
    '--db', 'test',
    '--collection', 'source',
    '--projecionFile', 'jstests/export/testdata/simple_projection_file']
  ...
  // make sure only the specified projections were exported
