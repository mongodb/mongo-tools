 ...
 *    with minimal local implementations. These symbols are only referenced on code paths that
 *    the tools ShardingTest/ReplSetTest usage does not exercise when standing up a cluster
 *    (pre-image failpoints, oplog-truncation consistency checks); injectHookAppName wraps every
 *    connection, so it is provided as a pass-through. This mirrors how the 8.1 libs comment out
 *    unused server-only imports rather than vendoring the full transitive closure.
 * 3. Remaining `await import("jstests/...")` calls are left as-is: they live in code paths (e.g.
 *    validate-collections-on-shutdown) that are not reached by the tools tests, matching the 8.1
 *    libs.
 */
import {Thread} from "../shell_common/libs/parallelTester-9.0.js";

// injectHookAppName (jstests/libs/hook_appname.js) normally appends a test-hook appName to the
// connection string. The tools tests don't rely on hook appNames, so pass the host through
// unchanged.
function injectHookAppName(host) {
    return host;
}

// configureFailPoint (jstests/libs/fail_point_util.js). Faithful minimal implementation so it
// behaves correctly if ever reached, though the tools tests do not enable these failpoints.
function configureFailPoint(conn, failPointName, data = {}, mode = "alwaysOn") {
    const res = assert.commandWorked(
        conn.adminCommand({configureFailPoint: failPointName, mode: mode, data: data}));
    return {
        conn: conn,
        failPointName: failPointName,
        timesEntered: res.count,
        wait: function({maxTimeMS = 5 * 60 * 1000, timesEntered = res.count, expectedCount = 1} = {}) {
            assert.commandWorked(conn.adminCommand({
                waitForFailPoint: failPointName,
                timesEntered: timesEntered + expectedCount,
                maxTimeMS: maxTimeMS,
            }));
        },
        off: function() {
...
// replicated_truncates_utils.js helpers gate optional oplog-truncation consistency checks that
// the tools tests do not require; report them as not mandated so the checks are skipped.
function featureFlagMandatesReplicatedTruncates(_conn) {
    return false;
}
function persistenceProviderRequiresReplicatedTruncates(_conn, _liveNodes) {
    return false;
}
...
export class ReplSetTest {
    ...
    start(n, options, restart, waitForHealth) {
        ...
        options.setParameter.initialSyncWaitForSyncSourceLastStableRecoveryTsInitiatingSetThresholdSecs =
            options.setParameter
                .initialSyncWaitForSyncSourceLastStableRecoveryTsInitiatingSetThresholdSecs || 0;
