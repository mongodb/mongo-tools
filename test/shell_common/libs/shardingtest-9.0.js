 ...
 *    paths the tools ShardingTest usage does not exercise (sharding a timeseries collection,
 ...
 */
// getTimeseriesCollForDDLOps (jstests/core/timeseries/libs/viewless_timeseries_util.js) resolves
// the namespace to target for DDL ops on a timeseries collection. The tools ShardingTest usage
// only shards ordinary collections, so return the collection unchanged.
function getTimeseriesCollForDDLOps(db, coll) {
    return coll;
}

// FeatureFlagUtil (jstests/libs/feature_flag_util.js). Only isPresentAndEnabled/getFeatureFlagDoc
// are referenced, on code paths the tools tests do not exercise. Report flags as disabled to
// preserve pre-9.0 behavior.
const FeatureFlagUtil = {
    isPresentAndEnabled: function() {
        return false;
    },
    getFeatureFlagDoc: function() {
        return {version: "0.0"};
    },
};
import {Thread} from "../shell_common/libs/parallelTester-9.0.js";
import {ReplSetTest} from "../shell_common/libs/replsettest-9.0.js";
