...
//SessionStub mocks an MongoSession by implementing the AcquireSocketPrivate method.
//It allows for tests to pass around a struct with stubbed fields that can then be read later for testing
...
//ConnStub mocks the connection used by an mgo session. It implements the net.Conn
//interface so that it may be used as a connection for testing in llmgo
//It contains a write buffer and a read buffer. It writes into the write buffer, and reads from the read buffer
//so that its ends may be given in reverse to another function. (i.e., another function can write to its read buffer
//and it will receive this as incoming data)
...
//newTwoSidedConn makes two ConnStub's which use the same buffers but in opposite roles.
//The read end of one buffer is handed to the other connection as the write end and vice versa
...
//AcquireSocketPrivate is an implementation of MongoSession's function that allows for
