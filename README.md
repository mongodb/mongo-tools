...
##### Use cases
- Preview how well your database cluster would perform a production workload under a different environment settings are modified (storage engine, index, hardware, OS, etc.)
- Reproduce and investigate bugs by replaying 
...
`mongoplay` works in two phases: `record` and `play`. 
...
* The `play` reads in a playback file that was generated by `record`, and re-executes the workload against some target host. 
