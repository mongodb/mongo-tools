...
You can also build a subset of the tools using the `-tools` option. For example, `./make build -pkgs=mongodump,mongorestore` builds only `mongodump` and `mongorestore`.
