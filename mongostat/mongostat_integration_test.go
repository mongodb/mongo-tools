...
// validation and calls os.Exit, so these tests build the tool once and run it
// the way the JS tests they replace did, reading its exit status and output.
...
//
// The JS test also asserted that --rowcount -1 exits nonzero. It does not:
// --rowcount is documented as "0 for indefinite" and limitableFormatter treats
// every value <= 0 that way, so a negative count runs forever by design. That
// assertion only passed because the invocation left out --host and --port and so
// failed to connect at all, and is not carried over.
func TestMongostatRowCount(t *testing.T) {
	...
	t.Run("prints the requested number of rows", func(t *testing.T) {
		...
		assert.Len(t, statRows(stdout), rowCount, "--rowcount is the number of rows printed")
	})

	t.Run("a rowcount that is not a number is rejected", func(t *testing.T) {
		_, stderr, err := runMongostat(t, "--rowcount", "foobar")
		assert.Error(t, err, "mongostat exits nonzero")
	...
	t.Run("a server that cannot be reached is an error", func(t *testing.T) {
		...
		_, stderr, err := runMongostatArgs(t, slices.Concat(
	...
	t.Run("a replica set name that does not match is an error", func(t *testing.T) {
		_, stderr, err := runMongostatArgs(t, slices.Concat(
			testutil.GetSSLArgs(), testutil.GetAuthArgs(),
			[]string{
				"--host", "badreplset/" + testServerHostPort(t),
				"--serverSelectionTimeout", "2",
				"--rowcount", "1",
			},
		)...)
...
func TestMongostatSleepTime(t *testing.T) {
	...
	assert.Len(t, statRows(stdout), rowCount, "every row is printed")
...
func TestMongostatExitsOnSignal(t *testing.T) {
	...
	cmd := exec.Command(mongostatBinary(t), append(testutil.GetBareArgs(), "--noheaders")...)
	...
	case row, ok := <-firstRow:
		require.True(t, ok, "mongostat printed a row before being signaled: %s", stderr.String())
		require.NotEmpty(t, strings.Fields(row), "the row has content")
	case <-time.After(30 * time.Second):
		require.NoError(t, cmd.Process.Kill(), "can kill a mongostat that never reported")
		t.Fatalf("mongostat printed no rows: %s", stderr.String())
	}
...
func TestMongostatAuth(t *testing.T) {
	...
	t.Run("the right password succeeds", func(t *testing.T) {
		stdout, stderr, err := runMongostatArgs(t,
			slices.Concat(args, []string{"--password", auth.Password})...)
		require.NoError(t, err, "mongostat exits successfully: %s", stderr)
		assert.Len(t, statRows(stdout), 2, "a header and one data row are printed")
	})
	...
	t.Run("a wrong password fails", func(t *testing.T) {
		...
		assert.Empty(t, statRows(stdout), "no stats are printed")
	})
}
...
func TestMongostatDiscoverShards(t *testing.T) {
	...
	for _, row := range statRows(stdout) {
...
// statRows returns the non-empty lines of mongostat's stat output. Both the
// header and the data rows are padded, so they are returned trimmed.
func statRows(stdout string) []string {
	var rows []string
...
func runMongostat(t *testing.T, args ...string) (string, string, error) {
	t.Helper()
	return runMongostatArgs(t, append(testutil.GetBareArgs(), args...)...)
}

func runMongostatArgs(t *testing.T, args ...string) (string, string, error) {
	t.Helper()
	cmd := exec.Command(mongostatBinary(t), args...)
...
// testServerHostPort is the test server as a host:port, for the cases that
// supply their own credentials and so cannot use the arguments GetBareArgs
// builds.
