...
func New(opts Options) (*MongoFiles, error) {
	...
	if err != nil {
		return nil, util.SetupError{Err: fmt.Errorf("error connecting to host: %v", err), Code: util.ExitError}
	}
	...
	if err := mf.ValidateCommand(opts.ParsedArgs); err != nil {
		return nil, util.SetupError{Err: err, Code: util.ExitBadOptions}
	}
