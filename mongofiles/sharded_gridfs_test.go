...
// so that the file is stored as many GridFS chunks. Whether those chunks end up
// on more than one shard is up to the balancer and the cluster's chunk size,
// neither of which this test controls.
...
func writeLargeLocalFile(t *testing.T) string {
	...
	require.NoError(t, os.WriteFile(path, contents, 0o600), "can write the file to upload")
...
func runShardedMongofiles(
	...
	mf, err := simpleMongoFilesInstanceWithFilename(t, command, gridFSName)
