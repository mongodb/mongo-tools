...
func (restore *MongoRestore) ReadPreludeMetadata(target archive.DirLike) error {
	...
	if err != nil {
		...
		if err != nil {
			log.Logvf(log.Always, "error in prelude restore: 1 %v", err)
	...
	if restore.InputOptions.Gzip {
		...
		if err != nil {
			log.Logvf(log.Always, "error in prelude restore: 5 %v", err)
	...
	if err != nil {
		log.Logvf(log.Always, "error in prelude restore: 2 %v", err)
	...
	if err != nil {
		log.Logvf(log.Always, "error in prelude restore: 3 %v", err)
	...
	if !ok {
		log.Logvf(log.Always, "error in prelude restore: 4 %v", err)
