...
func TestReadDumpServerVersionFromArchive(t *testing.T) {
	...
	withArchiveMongodump(t, func(archivePath string) {
		...
		args := []string{
			...
			// ParseOptions calls log.SetVerbosity with the options it parsed,
			// which resets the global verbosity this package's init() raised, so
			// the debug lines below are only emitted if the restore asks for them.
			"-vvv",
		}
		restore, err := getRestoreWithArgs(args...)
		require.NoError(err)
		...
		// The three versions read out of the archive prelude are only reported
		// through the log, so the log is the only place they can be checked.
