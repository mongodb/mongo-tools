...
- `mongofiles_version.js` — NEW (no Go test verifies `--version` output; goes in `common/options/options_test.go` to cover all tools at once)
...
- `version_test.js` — NEW (no Go test verifies that mongorestore with `-vvv` on an archive dump prints archive format version, server version, and tool version)
...
- [ ] **Step 18: Skip the following:**
  ...
  - `version_test.js` → convert: `TestRestoreArchiveVersionStrings` in `mongorestore/mongorestore_qa_test.go`: dump one doc to an archive, restore with `-vvv`, verify output contains archive format version, server version, and tool version strings
...
- `different_collection.js` — EXTEND (TestMongorestore covers nsFrom/nsTo same-DB rename but not: cross-database restore with --db/--collection, and --db/--collection with a direct .bson file path)
